"""
Legacy: FinBERT multi-task fine-tune (sentiment + reason).
Output: models/finbert-multitask/
"""
...
SAVE_PATH    = Path("models/finbert-multitask")
...
class FinBERTMultiTask(nn.Module):
    def __init__(self, bert, num_sentiment=3, num_reason=7):
        super().__init__()
        self.bert           = bert
...
def main():
    ...
    train_ds = NewsDataset(TRAIN_CSV, tokenizer)
    val_ds   = NewsDataset(VAL_CSV,   tokenizer)
    test_ds  = NewsDataset(TEST_CSV,  tokenizer)
