"""
A5 (NOT RUN) - A3 recipe on complete_v4 (complete_v3 + wave 2, train 37,965;
dataset later deleted). Imports from train_sentiment_v2.py.

Run:    caffeinate -i uv run scripts/train_sentiment_v5.py
Output: models/finbert-sentiment-v4-sqrt/ (+ train_log.txt)
"""
...
SAVE_PATH  = Path("models/finbert-sentiment-v4-sqrt")
LOG_PATH   = SAVE_PATH / "train_log.txt"
DATA_DIR   = Path("data/complete_v4")
...
def main():
    for p in (TRAIN_CSV, VAL_CSV, TEST_CSV):
        if not p.exists():
            raise SystemExit(f"No data: {p} — build complete_v4 with build_dataset_v3 --run run3 first")
    ...
    log(f"\n═══ {time.strftime('%Y-%m-%d %H:%M')}  A5  device={DEVICE}  data={DATA_DIR}  "
    ...
    log(f"\n── Sentiment Test Report (A5 v4 sqrt, best val macro_f1={best_macro_f1:.3f}, "
