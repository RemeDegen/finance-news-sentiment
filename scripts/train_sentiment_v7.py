"""
A7 - A3 recipe (sqrt weights, warmup 6% + linear decay, 5 epochs, best-val
checkpoint) on complete_v2; isolates the recipe effect against A2.
Result: test acc 0.845 / macro F1 0.809 = A2, but worse calibration -> A2 stays
final. Imports from train_sentiment_v2.py.

Run:    caffeinate -i uv run scripts/train_sentiment_v7.py
Output: models/finbert-sentiment-v2-sqrt-warmup/ (+ train_log.txt)
"""
...
SAVE_PATH  = Path("models/finbert-sentiment-v2-sqrt-warmup")
LOG_PATH   = SAVE_PATH / "train_log.txt"
DATA_DIR   = Path("data/complete_v2")
...
def main():
    ...
    log(f"\n═══ {time.strftime('%Y-%m-%d %H:%M')}  A7 (v2 data, A3 recipe)  device={DEVICE}  data={DATA_DIR}  "
    ...
    log(f"\n── Sentiment Test Report (A7 v2 sqrt+warmup, best val macro_f1={best_macro_f1:.3f}, "
